(() => {
  ...
  // Personalized GitHub Pages invitations.
  // Example: ?guest=دکتر+محمد+رضایی
  // The value is always written with textContent (never innerHTML).
  ...
  function initPersonalInvitation() {
    ...
    if (!guestName) {
      window.MAZUMSInvite = Object.freeze({
        guest: '',
        createLink(name) {
          const clean = normalizeGuestName(name);
          if (!clean) return location.href.split('#')[0].split('?')[0];
          const url = new URL(location.href);
          url.search = '';
          url.hash = '';
          url.searchParams.set('guest', clean);
          return url.href;
        }
      });
      return;
    ...
    const code = guestCode(guestName);
    body.classList.add('is-personalized');
    if (guestName.length > 34) body.classList.add('guest-name-long');

    document.title = `${guestName} · ${baseTitle}`;
    const description = `دعوت‌نامه اختصاصی ${guestName} برای آیین دانش‌آموختگی دانشجویان پزشکی ورودی بهمن ۱۳۹۸ دانشگاه علوم پزشکی مازندران.`;
    document.querySelector('meta[name="description"]')?.setAttribute('content', description);
    document.querySelector('meta[property="og:title"]')?.setAttribute('content', `${guestName} · دعوت‌نامه آیین دانش‌آموختگی`);
    document.querySelector('meta[property="og:description"]')?.setAttribute('content', description);

    if (envelopeRecipient) envelopeRecipient.textContent = guestName;
    if (envelopeRecipientNote) envelopeRecipientNote.textContent = 'این دعوت‌نامه به‌صورت اختصاصی برای شما آماده شده است';
    if (envelopePersonalMark) envelopePersonalMark.hidden = false;

    if (mailCardEyebrow) mailCardEyebrow.textContent = 'دعوت‌نامه رسمی و اختصاصی';
    if (mailCardGuest) mailCardGuest.hidden = false;
    if (mailCardGuestName) mailCardGuestName.textContent = guestName;
    if (mailCardGuestCode) mailCardGuestCode.textContent = code;
    if (mailCardLead) mailCardLead.textContent = 'با سپاس از سال‌های آموزش، راهنمایی و همراهی شما، افتخار داریم حضورتان را در این آیین گرامی بداریم.';
    mailCard?.setAttribute('aria-label', `دعوت‌نامه اختصاصی ${guestName}`);

    if (finaleEyebrow) finaleEyebrow.textContent = `به افتخار حضور ${guestName}`;
    if (finaleLead) finaleLead.textContent = 'آنچه امروز به نام دانش‌آموختگی پاس می‌داریم، حاصل سال‌هایی است که دانش و تجربه‌ی استادان به ما جهت داد و معنای مسئولیت در برابر جان انسان را روشن‌تر کرد. حضور گران‌قدر شما در این آیین، برای ما ادای احترام به همان سال‌هاست.';

    if (inviteCardKicker) inviteCardKicker.textContent = 'دعوت‌نامه اختصاصی · با سپاس و امتنان';
    if (inviteCardGuest) inviteCardGuest.hidden = false;
    if (inviteCardGuestName) inviteCardGuestName.textContent = guestName;
    if (inviteCardTitle) inviteCardTitle.textContent = 'با افتخار دعوت می‌کنیم';
    if (inviteCardLead) inviteCardLead.textContent = 'با نهایت احترام و امتنان، از شما دعوت می‌کنیم در آیین دانش‌آموختگی دانشجویان پزشکی دانشگاه علوم پزشکی مازندران حضور یابید. حضور گران‌قدر شما برای ما نه‌تنها موجب افتخار، بلکه فرصتی برای ادای سپاس به سال‌هایی است که با دانش، راهنمایی و اعتماد شما شکل گرفت.';
    if (inviteEdition) inviteEdition.textContent = `نسخه اختصاصی · ${code}`;
    inviteStage?.setAttribute('aria-label', `دعوت‌نامه رسمی و اختصاصی ${guestName}`);
  ...
  function initMailEntrance() {
    ...
    if (invitationParams.get('entrance') === 'skip') {
      mailIntro.remove();
      body.classList.remove('invitation-locked');
      body.classList.add('invitation-entered');
      startAutoScroll({ reset: true, force: true });
